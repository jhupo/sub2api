    ...
    <div class="space-y-5 pb-12">
      <section class="glass-card overflow-hidden p-0">
        <header class="flex flex-wrap items-start justify-between gap-4 border-b border-gray-100 px-5 py-4 dark:border-dark-700 sm:px-6">
          <div class="min-w-0">
            ...
            <div class="mt-1.5 flex flex-wrap items-center gap-2 text-xs text-gray-500 dark:text-gray-400">
              ...
              <span>{{ snapshot ? t('channelMonitorV2.updatedTo', { time: formatTime(snapshot.coverage.data_through) }) : t('common.loading') }}</span>
              <span v-if="snapshot && !snapshot.coverage.coverage_complete" class="badge badge-warning">{{ t('channelMonitorV2.partialCoverage') }}</span>
            </div>
          </div>
        ...
        <div class="flex flex-wrap items-center gap-2 px-4 py-3 sm:px-5">
          ...
          <span class="text-xs text-gray-500 dark:text-gray-400">{{ t('channelMonitorV2.compact.description') }}</span>
          <span v-if="snapshot" class="ml-auto text-xs font-medium tabular-nums text-gray-500 dark:text-gray-400">{{ t('channelMonitorV2.compact.summary', { success: formatPercent(1 - (latestSnapshotMetrics?.error_rate ?? 0)), cache: formatPercent(latestSnapshotMetrics?.cache_rate ?? 0) }) }}</span>
        </div>
      </section>
      ...
      <EmptyState v-else-if="rows.length === 0" :title="t('channelMonitorV2.compact.emptyTitle')" :description="t('channelMonitorV2.compact.emptyDescription')" />
      <div v-else class="grid grid-cols-1 gap-5 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">
        <ChannelMonitorV2Card
          v-for="row in rows"
...
import ChannelMonitorV2Card from '@/components/user/monitor/ChannelMonitorV2Card.vue'
...
async function reload(silent = true) {
  ...
  try {
    ...
    if (e.name !== 'AbortError' && e.code !== 'ERR_CANCELED') appStore.showError(extractApiErrorMessage(error, t('channelMonitorV2.loadFailed')))
  } finally {
