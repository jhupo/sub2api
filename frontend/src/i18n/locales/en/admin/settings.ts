export default {
  ...
  "ttlHint": "1–60 minutes, default 40. A parseable embedded timestamp supplies the one-hour lifetime; this interval is used only as the fallback when no timestamp can be parsed.",
  "length": "Expected state length (bytes)",
  "lengthHint": "Default 292. Values 16 bytes longer are flagged as possibly degraded; other lengths are invalid. Neither replaces a normal value.",
  "ttlInvalid": "The State fallback rotation interval must be an integer from 1 to 60 minutes.",
  "lengthInvalid": "The expected State length must be an integer from 1 to 8192.",
  "autoReplace": "Automatically replace State",
  "autoReplaceHint": "Only opted-in account/model pairs are processed. A State enters the serial background queue ten minutes before expiry and is replaced atomically only after a successful refresh. Manual refreshes remain available.",
  ...
  "webshareCountryModeHint": "Global random leaves country selection to Webshare. A specified pool randomly selects one configured country for each refresh.",
  ...
  "saveHint": "Disabling the feature or changing the length standard clears the cache. Automatic replacement, rotation timing, and Webshare settings affect future refreshes. Multi-instance propagation takes up to about three seconds.",
  "save": "Save settings",
  ...
  "poolHint": "Only {length}-byte States are used. A parseable Fernet timestamp expires one hour after issue; otherwise the {ttl}-minute fallback interval from System Settings applies.",
  "upstreamExpiry": "Upstream expiry",
  "issued": "Issued",
  ...
  "stateInputHint": "Must be exactly {length} bytes. A parseable Fernet timestamp supplies the actual expiry.",
  ...
  "expirySource": {
    "fernet": "Fernet expiry",
    "fallback": "Fallback interval"
  },
