export default {
    settings: {
      upstreamState: {
  "retryIntervalInvalid": "失败重试间隔必须是 1–60 分钟的整数。",
  "retryIntervalHint": "1–60 分钟，默认 5。从本次刷新结束起计算，间隔结束后重新排队；实际执行受每分钟检查和队列等待影响。新取得的 State 已在轮换窗口内时也使用此间隔，避免反复刷新。",
  "retryInterval": "失败重试间隔（分钟）",
  "rotationLeadInvalid": "提前轮换时间必须是 0–30 分钟的整数。",
  "rotationLeadHint": "0–30 分钟，默认 10。可解析签发时间时，在预计到期前提前刷新，与自动轮换周期取更早者。0 表示不提前；无法解析时间戳时按轮换周期刷新。",
  "rotationLead": "提前轮换时间（分钟）",
  "title": "状态管理",
  "description": "每个 OpenAI OAuth 账号与最终出站模型维护一份 X-Codex-Turn-State；支持手动设置、立即刷新和到期自动替换。",
  "back": "返回功能开关",
  "configure": "打开状态管理",
  "refresh": "刷新列表",
  "settings": "采集与复用设置",
  "scopeHint": "按 OpenAI OAuth 账号与最终出站模型采集和复用 State；同一账号的不同模型分别维护。",
  "enabled": "启用状态管理",
  "enabledHint": "默认关闭。开启后左侧显示“状态管理”，可按账号和模型单独启用替换；关闭后入口隐藏并恢复正常转发。",
  "ttl": "自动轮换周期（分钟）",
  "ttlHint": "1–60 分钟，默认 40。每次成功获取 State 后开始计时，到达此周期就排队获取下一份；可解析时间戳时，若提前轮换时间更早则优先刷新。修改周期也适用于已有 State。",
  "length": "预期 state 长度（字节）",
  "lengthHint": "默认 292。只接受符合长度与格式校验的值；长度不符不会替换已保存的 State。长度本身不能证明模型质量。",
  "ttlInvalid": "自动轮换周期必须是 1–60 分钟的整数。",
  "lengthInvalid": "State 预期长度必须是 1–8192 的整数。",
  "autoReplace": "自动替换 State",
  "autoReplaceHint": "仅处理已启用替换的账号与模型。每分钟检查一次，无可用 State、达到轮换周期或进入预计到期前 {lead} 分钟时加入串行队列；失败等待 {retry} 分钟后再排队，无次数上限。失败保留未过期的旧 State；手动刷新不受自动开关和等待间隔限制。",
  "autoReplaceOn": "自动替换已开启",
  "autoReplaceOff": "自动替换已关闭",
  "webshareEnabled": "使用 Webshare Rotating Residential 刷新 State",
  "webshareHint": "仅主动刷新 State 的 SSE 请求使用 Webshare。每次按单条分页随机选择一份代理凭据，通过 p.webshare.io:80 发起一次请求；普通业务请求仍使用账号原有出口。",
  "webshareApiKey": "Webshare Token API Key",
  "webshareApiKeyHint": "用于 Authorization: Token 鉴权，只保存不回显。留空会保留已保存的密钥。",
  "webshareKeyConfigured": "已配置；留空保持不变",
  "webshareKeyPlaceholder": "粘贴 Webshare Token API Key",
  "webshareKeyRequired": "启用 Webshare 刷新前必须配置 API Key。",
  "webshareCountryMode": "出口国家模式",
  "webshareCountryRandom": "全球随机",
  "webshareCountrySpecified": "指定国家池",
  "webshareCountryModeHint": "全球随机从可用代理条目中等概率抽取，并非各国家等概率；指定国家池先等概率选择一个国家，再随机选择该国条目。不保证每次出口 IP 不同。",
  "webshareCountries": "国家代码",
  "webshareCountriesHint": "填写 ISO 3166-1 两字母代码，可用逗号或空格分隔，例如 US, JP, DE。",
  "webshareCountriesInvalid": "指定国家池至少需要一个有效的两字母国家代码。",
  "saveHint": "关闭总开关或修改长度标准会清空缓存。轮换周期、提前轮换时间和重试间隔会用于下一次调度检查，不会延长已保存 State 的有效期。多实例配置最多约 3 秒同步。",
  "save": "保存设置",
  "cached": "缓存状态",
  "cacheHint": "仅展示摘要，最多保存 4096 条；由手动设置、立即刷新或自动轮换更新，普通业务响应不更新 State。",
  "clearAll": "清空全部",
  "disabled": "功能未启用，未进行主动刷新或复用。",
  "loading": "正在加载状态…",
  "empty": "暂无状态。可手动设置、立即刷新，或开启自动替换获取 State。",
  "unavailable": "列表暂不可用，请刷新重试。",
  "account": "账号",
  "modelSession": "出站模型",
  "summary": "State 摘要 / 长度",
  "egress": "采集出口",
  ...
  "models": "覆盖模型",
  "expiring": "5 分钟内过期",
  "loadFailed": "设置加载失败，请刷新后重试。",
  "listFailed": "状态列表加载失败，当前显示可能已过期，请刷新重试。",
  "saved": "状态管理设置已保存",
  "saveFailed": "保存失败。若其他页面已修改配置，请刷新页面后重试。",
  "clearConfirm": "清空这条缓存？之后可手动刷新，或由已开启的自动替换重新获取。",
  "clearAllConfirm": "清空全部缓存？之后可手动刷新，或由已开启的自动替换重新获取。",
  "clearFailed": "清空失败，请重试。",
  "expired": "已过期",
  "matrix": "OAuth 账号与模型",
  "matrixHint": "按账号和最终出站模型逐项启用，默认关闭。模型来自现有模型清单和账号映射，不代表账号拥有调用权限。关闭单项只清除该账号与模型的 State。",
  "model": "出站模型",
  "pairEnabled": "启用替换",
  "check": "最近长度检查",
  "checked": "检查时间",
  ...
  "missing": "未返回 state",
  "invalid": "Fernet 格式无效",
  "poolTitle": "账号 / 模型 State 池",
  "poolHint": "只有长度为 {length} 且校验通过的 State 才用于请求。成功获取后每 {ttl} 分钟轮换；可解析时间戳时按签发后一小时估算到期，如到期前提前轮换更早则优先刷新。失败按系统设置的间隔重试。",
  "upstreamExpiry": "预计到期倒计时",
  "issued": "签发",
  "rotateAt": "下次轮换",
  "stateUnavailable": "尚未保存可用 State",
  "setState": "设置 State",
  "setStateTitle": "设置 X-Codex-Turn-State",
  "statePlaceholder": "粘贴完整的 gAAAAA… State",
  "stateInputHint": "必须恰好为 {length} 字节；可解析 Fernet 签发时间时，按签发后一小时估算到期。",
  "stateLength": "State 长度",
  "filterAccounts": "筛选账号",
  "onlyEnabled": "仅看已启用替换",
  "noEnabledPairs": "暂无匹配的已启用项。取消“仅看已启用替换”可查看全部账号和模型并启用替换。",
  "filterModels": "筛选模型",
  ...
  "stateSet": "State 已设置",
  "stateSetFailed": "State 设置失败",
  "refreshNow": "立即刷新",
  "refreshSucceeded": "已从上游刷新 State",
  "refreshFailed": "上游 State 刷新失败",
  "lastRefreshError": "最近刷新失败",
  "status": {
    "normal": "可用",
    "extended": "疑似降智（+16B）",
    "mismatch": "长度异常",
