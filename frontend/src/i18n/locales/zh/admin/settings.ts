export default {
  ...
  "ttl": "自动轮换周期（分钟）",
  "ttlHint": "1–60 分钟，默认 40。优先使用 State 内嵌时间戳计算一小时有效期；无法解析时间戳时才使用此周期保底轮换。",
  "length": "预期 state 长度（字节）",
  "lengthHint": "默认 292。多 16 字节会标记为疑似降智，其他长度标记为异常，均不会替换正常值。",
  "ttlInvalid": "State 保底轮换周期必须是 1–60 分钟的整数。",
  "lengthInvalid": "State 预期长度必须是 1–8192 的整数。",
  "autoReplace": "自动替换 State",
  "autoReplaceHint": "仅处理已启用替换的账号与模型。State 进入到期前 10 分钟后加入后台串行队列，刷新成功才原子替换；失败时保留旧 State。手动刷新不受此开关限制。",
  ...
  "webshareCountryModeHint": "全球随机不添加国家参数，由 Webshare 从套餐可用池选择；指定国家池会在每次刷新时随机选择一个国家。",
  ...
  "saveHint": "关闭总开关或修改长度标准会清空缓存；自动替换、轮换周期与 Webshare 配置只影响后续刷新。多实例配置最多约 3 秒同步。",
  "save": "保存设置",
  "cached": "缓存状态",
  ...
  "poolHint": "只有长度为 {length} 的 State 才会用于请求。可解析 Fernet 时间戳时按签发后一小时到期；无法解析时按系统设置中的 {ttl} 分钟保底周期轮换。",
  "upstreamExpiry": "上游到期倒计时",
  "issued": "签发",
  "rotateAt": "下次轮换",
  ...
  "stateInputHint": "必须恰好为 {length} 字节；可解析的 Fernet 时间戳用于计算真实到期时间。",
