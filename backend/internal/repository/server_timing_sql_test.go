...
func TestServerTimingConnectorRecordsDriverCallsWithoutRowLifetime(t *testing.T) {
	...
	gapMillis := float64(time.Since(gapStart)) / float64(time.Millisecond)
	...
	// Driver sleeps can overshoot on busy hosts. Compare app time with the
	// measured processing gap, not with the independently scheduled DB calls.
