...
func (s *AccountTestService) testClaudeAccountConnection(c *gin.Context, account *Account, modelID string) error {
	...
	c.Writer.Header().Set("Cache-Control", streamingCacheControlValue)
...
func (s *AccountTestService) testOpenAIAccountConnection(c *gin.Context, account *Account, modelID string, prompt string, mode string) error {
	...
	if isOAuth {
		...
		resolveCodexRequestClientIdentity("", credentialAccount.GetOpenAIUserAgent(), s.cfg != nil && s.cfg.Gateway.ForceCodexCLI).applyHeaders(req.Header)
	...
	if isOAuth && s.accountRepo != nil {
		if updates, err := codexUsageUpdatesFromResponse(resp); err == nil && len(updates) > 0 {
...
func (s *AccountTestService) testOpenAICompactConnection(c *gin.Context, account *Account, testModelID string) error {
	...
	if isOAuth {
		...
		if fpIDs := resolveCodexFingerprintIDsForInput(credentialAccount, extractCodexIdentityInput(req.Header, nil), 0, account.GetCodexFingerprintMode()); fpIDs != nil {
	...
	if s.accountRepo != nil {
		...
		if codexUpdates, err := codexUsageUpdatesFromResponse(resp); err == nil && len(codexUpdates) > 0 {
