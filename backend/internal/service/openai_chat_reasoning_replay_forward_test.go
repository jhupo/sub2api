...
func TestForwardAsChatCompletions_ReasoningReplayAcrossToolTurns(t *testing.T) {
	...
	raw, ok := stored[chatReasoningReplayCacheKey(chatReasoningReplayScope(c1, account.ID, turn1), "call_ls_1")]
...
func TestForwardAsChatCompletions_ReasoningReplayRetriesWithoutReplayOnRejectedCipher(t *testing.T) {
	...
	cache := &reasoningRecordingCache{getResp: map[string]string{chatReasoningReplayCacheKey(chatReasoningReplayScope(nil, 9, nil), "call_ls_1"): string(record)}}
