...
func TestRelayOpenAICodexTurnState_SetsHeaderAndRecordsProvenance(t *testing.T) {
	...
	raw, ok := svc.openaiCodexTurnStateOrigins.Load(openAICodexTurnStateOriginKey(c, "blob-A"))
	require.True(t, ok)
...
func TestRelayOpenAICodexTurnState_ClearsStaleValueWhenUpstreamAbsent(t *testing.T) {
	...
	_, ok := svc.openaiCodexTurnStateOrigins.Load(openAICodexTurnStateOriginKey(c, "blob-old"))
	require.False(t, ok)
}
...
func TestStageOpenAICodexTurnState_StagedHeaders(t *testing.T) {
	...
	_, noted := svc.openaiCodexTurnStateOrigins.Load(openAICodexTurnStateOriginKey(c, "blob-B"))
	...
	raw, ok := svc.openaiCodexTurnStateOrigins.Load(openAICodexTurnStateOriginKey(c, "blob-B"))
	require.True(t, ok)
	origin, ok := raw.(openAICodexTurnStateOrigin)
	require.True(t, ok)
...
func TestNoteStagedOpenAICodexTurnStateCommitted_NoopWithoutState(t *testing.T) {
	...
	_, ok := svc.openaiCodexTurnStateOrigins.Load(openAICodexTurnStateOriginKey(c, ""))
	require.False(t, ok)
}
...
func TestGuardOpenAICodexTurnStateEcho(t *testing.T) {
	...
	t.Run("expired_provenance_passthrough_and_pruned", func(t *testing.T) {
		...
		svc.openaiCodexTurnStateOrigins.Store(openAICodexTurnStateOriginKey(c, "blob-A"), openAICodexTurnStateOrigin{
			accountID: 42,
		...
		svc.guardOpenAICodexTurnStateEcho(c, &Account{ID: 43}, h)
		require.Equal(t, "blob-A", h.Get("x-codex-turn-state"))
		_, ok := svc.openaiCodexTurnStateOrigins.Load(openAICodexTurnStateOriginKey(c, "blob-A"))
		require.False(t, ok)
	})
