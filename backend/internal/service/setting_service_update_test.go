...
func (s *defaultSubPlanReaderStub) GetPlan(ctx context.Context, id int64) (*SubscriptionPlan, error) {
...
func TestSettingService_UpdateSettings_DefaultSubscriptions_RejectsMissingPlan(t *testing.T) {
	...
	planReader := &defaultSubPlanReaderStub{
		errBy: map[int64]error{
			12: errors.New("subscription plan not found"),
		},
	}
	svc := NewSettingService(repo, &config.Config{})
	svc.SetDefaultSubscriptionPlanReader(planReader)
...
func TestSettingService_UpdateSettings_DefaultSubscriptions_RejectsDuplicatePlanWithoutPlanReader(t *testing.T) {
	repo := &settingUpdateRepoStub{}
	svc := NewSettingService(repo, &config.Config{})
