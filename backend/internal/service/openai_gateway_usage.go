...
func (s *OpenAIGatewayService) RecordUsage(ctx context.Context, input *OpenAIRecordUsageInput) error {
	...
	// Apply all normal pricing modifiers first, then make this the final cost
	// guard so no later policy can accidentally make the failed request billable.
	...
	if !isSubscriptionBilling {
		// Do not attribute a wallet-funded event to a stale subscription object.
		subscription = nil
	}
