...
func (s *AccountUsageService) getOpenAIUsage(ctx context.Context, account *Account, force bool) (*UsageInfo, error) {
	...
	// Repository/cache snapshots can be shared by concurrent dashboard calls.
	accountCopy := *account
...
func (s *AccountUsageService) shouldQueryOpenAICodexSnapshot(accountID int64, now time.Time, force ...bool) bool {
	if s == nil || s.cache == nil || accountID <= 0 {
		return true
	}
	forceProbe := len(force) > 0 && force[0]
	interval := openAIProbeCacheTTL
	if forceProbe {
...
func (s *AccountUsageService) queryOpenAICodexSnapshot(ctx context.Context, account *Account) (map[string]any, error) {
	if account == nil || !account.IsOAuth() {
		return nil, nil
	}
	if s.openAIQuotaService == nil {
		return nil, fmt.Errorf("OpenAI quota service unavailable")
	}
	usage, err := s.openAIQuotaService.QueryUsage(ctx, account.ID)
	if err != nil {
		return nil, err
	}
	updates := buildCodexQuotaUsageUpdates(usage, time.Now())
	if account.IsShadow() {
		updates = buildCodexSparkWindowExtraUpdates(usage, time.Now())
	}
	if len(updates) == 0 {
		return nil, fmt.Errorf("OpenAI quota response has no usage windows")
	}
...
func applyExtraToUsage(usage *UsageInfo, extra map[string]any, now time.Time) {
	...
	if observedAt, err := parseTime(fmt.Sprint(extra["codex_usage_updated_at"])); err == nil {
...
func (s *AccountUsageService) getGeminiUsage(ctx context.Context, account *Account) (*UsageInfo, error) {
	...
	usage.GeminiSharedDaily, usage.GeminiProDaily, usage.GeminiFlashDaily = buildGeminiUsageWindows(stats, quota.SharedRPD, quota.ProRPD, quota.FlashRPD, dailyResetAt, now)
	...
	usage.GeminiSharedMinute, usage.GeminiProMinute, usage.GeminiFlashMinute = buildGeminiUsageWindows(minuteStats, quota.SharedRPM, quota.ProRPM, quota.FlashRPM, minuteResetAt, now)
...
func buildCodexUsageProgressFromExtra(extra map[string]any, window string, now time.Time) *UsageProgress {
	...
	if resetAtRaw, ok := extra[resetAtKey]; ok {
		if resetAt := parseExtraTime(resetAtRaw); !resetAt.IsZero() {
	...
	if progress.ResetsAt == nil {
		if resetAfterSeconds := parseExtraInt(extra[resetAfterKey]); resetAfterSeconds > 0 {
			...
			if updatedAtRaw, ok := extra["codex_usage_updated_at"]; ok {
				if updatedAt := parseExtraTime(updatedAtRaw); !updatedAt.IsZero() {
					base = updatedAt
				}
			}
...
func buildGeminiUsageWindows(stats []usagestats.ModelStat, sharedLimit, proLimit, flashLimit int64, resetAt, now time.Time) (shared, pro, flash *UsageProgress) {
	var proStats, flashStats WindowStats
	for _, stat := range stats {
		target := &proStats
		if sharedLimit <= 0 && geminiModelClassFromName(stat.Model) == geminiModelFlash {
			target = &flashStats
		}
		target.Requests += stat.Requests
		target.Tokens += stat.TotalTokens
		target.Cost += stat.AccountCost
		target.StandardCost += stat.Cost
		target.UserCost += stat.ActualCost
	}
	if sharedLimit > 0 {
		return buildGeminiUsageProgress(sharedLimit, resetAt, proStats, now), nil, nil
	}
	return nil, buildGeminiUsageProgress(proLimit, resetAt, proStats, now), buildGeminiUsageProgress(flashLimit, resetAt, flashStats, now)
}

func buildGeminiUsageProgress(limit int64, resetAt time.Time, stats WindowStats, now time.Time) *UsageProgress {
	// limit <= 0 means "no local quota window" (unknown or unlimited).
	if limit <= 0 {
		return nil
	}
	utilization := (float64(stats.Requests) / float64(limit)) * 100
