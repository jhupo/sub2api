...
func openAIImagesToolUsageFromGJSON(value gjson.Result) (OpenAIUsage, bool) {
	...
	if !value.Get("output_tokens_details").Exists() {
		imageOutputTokens, imageOutputOK = outputTokens, outputOK
	}
	...
	return OpenAIUsage{
		...
		CacheReadInputTokens: min(openAICacheReadTokensFromUsage(value), inputTokens),
	}, true
}
