...
type transportRuntimeBlockerStub struct {
	calls  int
	id     int64
	until  time.Time
	reason string
...
func (b *transportRuntimeBlockerStub) BlockAccountScheduling(account *Account, until time.Time, reason string) {
	b.calls++
	b.id = account.ID
	b.until = until
	b.reason = reason
...
func (b *transportRuntimeBlockerStub) ClearAccountSchedulingBlock(int64) {}
...
func TestHandleUpstreamTransportError_PersistentEvictsAccount(t *testing.T) {
	repo := &transportTempUnschedRepoStub{}
	blocker := &transportRuntimeBlockerStub{}
	s := &GatewayService{
		accountRepo: repo,
	...
	if blocker.calls != 1 || blocker.id != account.ID || blocker.reason != "transport_error" {
		t.Fatalf("runtime block = calls:%d id:%d reason:%q, want one transport_error block for account %d", blocker.calls, blocker.id, blocker.reason, account.ID)
	}
	...
	if blocker.until.Before(wantUntil.Add(-time.Minute)) || blocker.until.After(wantUntil.Add(time.Minute)) {
		t.Fatalf("runtime until = %v, want ~%v", blocker.until, wantUntil)
	}
