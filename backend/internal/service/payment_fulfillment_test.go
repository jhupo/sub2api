...
func (c *paymentFulfillmentRedeemCacheStub) IncrementRedeemAttemptCount(context.Context, int64, time.Duration) error {
	c.incrementCalls++
	c.count++
	return nil
}
...
func TestResolveRedeemAction_LookupError(t *testing.T) {
	...
	require.ErrorContains(t, err, "lookup redeem code")
}
...
func TestExecuteBalanceFulfillmentRejectsCodeUsedByAnotherUser(t *testing.T) {
	...
	require.ErrorContains(t, err, "redeem code state does not match")
