...
func TestEnforceCodexIdentityHeadersWithAccountOverrideUA(t *testing.T) {
	...
	t.Run("低于版本门槛的 UA 整体回退", func(t *testing.T) {
		h := make(http.Header)
		h.Set("originator", "codex_cli_rs")
	...
	t.Run("显式 CLI UA 不跟随同步版本", func(t *testing.T) {
		...
		enforceCodexIdentityHeadersWithUA(h, "codex-tui/0.150.0 (Mac OS X 14.0; arm64) iTerm")

		require.Equal(t, "codex-tui", h.Get("originator"))
		require.Equal(t, "codex-tui/0.150.0 (Mac OS X 14.0; arm64) iTerm", h.Get("user-agent"))
		require.Equal(t, "0.150.0", h.Get("version"))
	})
}
