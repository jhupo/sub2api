...
const (
	...
	// Try the preferred model and at most one known compatibility model. This
	// avoids a false inconclusive result when a mapped model is unavailable,
	// while keeping probe cost and latency strictly bounded.
	codexQuotaOverdraftProbeAttemptLimit      = 2
	...
	codexQuotaOverdraftMaxInconclusiveRetries = 3
...
func (c *CodexQuotaOverdraftCoordinator) ObserveAccount(account *Account, preferredModel string) {
	...
	if _, loaded := c.observing.LoadOrStore(account.ID, struct{}{}); loaded {
		return
	}
	go func() {
		defer c.observing.Delete(account.ID)
		c.observeAccount(accountCopy, preferredModel)
	}()
}
...
func (c *CodexQuotaOverdraftCoordinator) ObserveBusinessSuccess(account *Account, preferredModel string) {
	...
	go func() {
		defer c.observing.Delete(account.ID)
		c.observeBusinessSuccess(accountCopy, preferredModel)
	}()
}
...
func (c *CodexQuotaOverdraftCoordinator) ObserveBusinessSuccessByID(accountID int64, preferredModel string) {
	...
	if _, loaded := c.observing.LoadOrStore(accountID, struct{}{}); loaded {
		return
	}
	go func() {
		defer c.observing.Delete(accountID)
...
func (c *CodexQuotaOverdraftCoordinator) observeBusinessSuccess(account *Account, preferredModel string) {
	...
	if current != nil && current.Status == codexQuotaOverdraftProbeInconclusive {
...
func (c *CodexQuotaOverdraftCoordinator) observeAccount(account *Account, preferredModel string) {
	...
	if state != nil && state.RecoverAt != nil && state.RecoverAt.After(now) {
		if state.Status == codexQuotaOverdraftProbeFailed {
			c.ensureFailedPause(account, state)
			return
		}
		if state.Status == codexQuotaOverdraftProbeInconclusive {
			if state.RetryAt == nil {
				retryAt := now.Add(time.Minute)
				state.RetryAt = &retryAt
				_ = c.persistNonFailedState(account.ID, state)
				return
			}
			if !state.RetryAt.After(now) {
				if signal, exhausted := codexQuotaOverdraftSignalFromAccount(account, state, now); exhausted {
...
func (c *CodexQuotaOverdraftCoordinator) HandleQuota429(
	...
	if snapshot := ParseCodexRateLimitHeaders(headers); snapshot != nil {
		...
		if err := c.accountRepo.UpdateExtra(persistCtx, account.ID, updates); err != nil {
			slog.Warn("codex_quota_overdraft_snapshot_persist_failed", "account_id", account.ID, "error", err)
		}
		cancel()
	}
...
func (c *CodexQuotaOverdraftCoordinator) startProbe(account *Account, signal codexQuotaOverdraftSignal, preferredModel string) {
	...
	if hasCurrent && codexQuotaOverdraftStateCoversSignal(current, signal) {
		...
		case codexQuotaOverdraftProbeInconclusive:
			if current.RetryCount >= codexQuotaOverdraftMaxInconclusiveRetries || current.RetryAt == nil || current.RetryAt.After(c.currentTime()) {
				return
			}
		}
	}
...
func (c *CodexQuotaOverdraftCoordinator) finishInconclusive(
	...
	state.TestedAt = &now
	state.RetryCount++
	if state.RetryCount >= codexQuotaOverdraftMaxInconclusiveRetries {
		state.RetryAt = nil
		if !c.persistNonFailedState(accountID, state) {
			return
		}
		slog.Warn("codex_quota_overdraft_probe_inconclusive_terminal",
			"account_id", accountID,
	...
	// Transient upstream/model errors must not permanently strand a quota cycle
	// in "inconclusive".  Retry with a bounded backoff; the coordinator still
	// requires the quota signal and the repository CAS before launching work.
	backoff := 30 * time.Second
	for i := 1; i < state.RetryCount && backoff < 15*time.Minute; i++ {
		backoff *= 2
	}
...
func (c *CodexQuotaOverdraftCoordinator) clearQuotaPause(accountID int64, state *CodexQuotaOverdraftProbeState) {
	...
	if clearer, ok := c.accountRepo.(codexQuotaOverdraftPauseClearer); ok {
		...
		// The runtime blocker is shared with auth/transport/capacity failures and
		// has no source-aware clear operation.  Leave it to its original expiry;
		// clearing it here could re-admit an account blocked for an unrelated
		// reason while the durable overdraft pause is being recovered.
		return
	}
	// Repositories without the CAS clearer still get the source-checked temp
	// pause cleanup.  Do not clear account.RateLimitResetAt here: that field is
	// shared by unrelated upstream 429s and cannot be attributed to overdraft.
	...
	// As above, do not clear the shared runtime blocker without a reason/CAS.
	// Its bridge cooldown is bounded by the same quota reset window.
}
...
func codexQuotaOverdraftSchedulingAllowed(account *Account, now time.Time) bool {
	...
	state, _ := codexQuotaOverdraftStateFromAccount(account)
	if state != nil && (state.Status == codexQuotaOverdraftProbeFailed || state.Status == codexQuotaOverdraftProbeInconclusive) {
		return false
	}
	signal, exhausted := codexQuotaOverdraftSignalFromAccount(account, state, now)
...
func codexQuotaOverdraftWindowResetAt(extra map[string]any, window string, now time.Time) *time.Time {
	if len(extra) == 0 {
		return nil
	}
	resetKey, afterKey := "", ""
	switch window {
	case "5h":
		resetKey, afterKey = "codex_5h_reset_at", "codex_5h_reset_after_seconds"
	case "7d":
		resetKey, afterKey = "codex_7d_reset_at", "codex_7d_reset_after_seconds"
	default:
		return nil
	}
	if resetAt := codexQuotaOverdraftResetAt(extra[resetKey], now); resetAt != nil {
		return resetAt
	}
	seconds := parseExtraFloat64(extra[afterKey])
	if seconds <= 0 {
		return nil
	}
