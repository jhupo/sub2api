...
func (s *GatewayService) handleUpstreamTransportError(ctx context.Context, c *gin.Context, account *Account, err error, event OpsUpstreamErrorEvent) error {
	...
	if account != nil && classifyUpstreamTransportError(err).Persistent {
...
// after a durable transport failure. The runtime block closes the cache
// propagation window; the database state remains the durable source of truth.
//
// Log semantics:
