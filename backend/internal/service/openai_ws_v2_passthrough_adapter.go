...
// captured session model in sync with the session-level model the client may
// rotate mid-session.
//
...
// the per-frame resolver returns "" and a stale captured session model falls
...
type openAIWSPassthroughActivityDeadline struct {
...
type openAIWSPassthroughActivityDeadlineState struct {
	armed      bool
	generation uint64
	deadline   openAIWSPassthroughActivityDeadline
}
...
type openAIWSPassthroughActivityFrameConn struct {
	limitAttempts     bool
	inner             openaiwsv2.FrameConn
	activeReadTimeout time.Duration

	mu              sync.Mutex
	state           openAIWSPassthroughActivityDeadlineState
	stageDownstream bool
	deadlineChanged chan struct{}
}

func (c *openAIWSPassthroughActivityFrameConn) ReadFrame(ctx context.Context) (coderws.MessageType, []byte, error) {
	if c == nil || c.inner == nil {
		return coderws.MessageText, nil, errOpenAIWSConnClosed
	}
	if ctx == nil {
...
func (c *openAIWSPassthroughActivityFrameConn) WriteFrame(ctx context.Context, msgType coderws.MessageType, payload []byte) error {
	if c == nil || c.inner == nil {
		return errOpenAIWSConnClosed
	}
	generation := uint64(0)
...
func (c *openAIWSPassthroughActivityFrameConn) Close() error {
	if c == nil || c.inner == nil {
		return nil
	}
	return c.inner.Close()
...
func (c *openAIWSPassthroughActivityFrameConn) armTurnDeadline() uint64 {
	if c == nil {
		return 0
	}
	c.mu.Lock()
	...
	c.state.armed = c.activeReadTimeout > 0
	c.state.deadline = openAIWSPassthroughActivityDeadline{timeout: c.activeReadTimeout, startedAt: time.Now()}
	c.stageDownstream = true
	c.mu.Unlock()
	c.notifyDeadlineChanged()
	return generation
}

func (c *openAIWSPassthroughActivityFrameConn) observeUpstreamActivity(msgType coderws.MessageType, payload []byte) {
	if c == nil {
		return
	}
...
func (c *openAIWSPassthroughActivityFrameConn) armActiveReadDeadline() {
	if c == nil {
		return
	}
	...
	c.state.generation++
	c.state.armed = true
	c.state.deadline = openAIWSPassthroughActivityDeadline{
...
func (c *openAIWSPassthroughActivityFrameConn) disarmDeadline(generation uint64) {
	if c == nil {
		return
	}
	c.mu.Lock()
	...
	c.state.armed = false
	if generation != 0 {
		c.stageDownstream = false
	}
	c.mu.Unlock()
	c.notifyDeadlineChanged()
}

func (c *openAIWSPassthroughActivityFrameConn) shouldCommitDownstream(msgType coderws.MessageType, payload []byte) bool {
	if c == nil {
		return true
	}
	c.mu.Lock()
	defer c.mu.Unlock()
	if !c.stageDownstream {
		return true
	}
	...
	// response.created exposes the response ID needed by response.cancel and is
	// therefore part of the public WS control protocol. Once it is forwarded,
	// this turn must not be replayed on another account. The independent
	// upstream-activity deadline remains armed until a terminal frame.
	committable := eventType == "response.created" ||
...
func (c *openAIWSPassthroughActivityFrameConn) deadlineState() openAIWSPassthroughActivityDeadlineState {
	if c == nil {
		return openAIWSPassthroughActivityDeadlineState{}
	}
	c.mu.Lock()
	defer c.mu.Unlock()
	return c.state
...
func (c *openAIWSPassthroughActivityFrameConn) notifyDeadlineChanged() {
...
var _ openaiwsv2.FrameConn = (*openAIWSPassthroughActivityFrameConn)(nil)
...
func (c *openAIWSClientFrameConn) WriteFrame(ctx context.Context, msgType coderws.MessageType, payload []byte) error {
	...
	if msgType == coderws.MessageText {
		...
		if eventType := strings.TrimSpace(gjson.GetBytes(payload, "type").String()); eventType == "error" || eventType == "response.failed" {
...
func (s *OpenAIGatewayService) proxyResponsesWebSocketV2Passthrough(
	...
	headers, stateResolution, buildHdrErr := s.buildOpenAIWSHeaders(
		ctx,
		c,
		account,
		token,
	...
	policyClientConn := &openAIWSPolicyEnforcingFrameConn{
		...
		// The filter and relay callbacks run in different goroutines; the
		// session-level model is synchronized through capturedSessionModelState.
		filter: func(msgType coderws.MessageType, payload []byte) (out []byte, blocked *OpenAIFastBlockedError, filterErr error) {
			...
			// 在评估策略前先刷新 captured session model：客户端可能通过
	...
	relayResult, relayExit := openaiwsv2.RunEntry(openaiwsv2.EntryInput{
		...
		Options: openaiwsv2.RelayOptions{
			...
			DownstreamStage: &openaiwsv2.DownstreamStageOptions{
				ShouldCommit: relayUpstreamFrameConn.shouldCommitDownstream,
				MaxBytes:     openAIFirstOutputStageMaxBytes,
			},
			StartsVisibleOutput: func(payload []byte) bool {
				eventType := strings.TrimSpace(gjson.GetBytes(payload, "type").String())
				return openAIStreamDataStartsVisibleOutput(string(payload), eventType)
			},
			...
			BeforeRelayCancel: func(exit openaiwsv2.RelayExit) {
				...
				status, reason, ok := openAIWSPassthroughRelayClientClose(exit)
				if !ok {
					return
				}
				// A raw upstream close while a response is in flight used to expose
				// only a close frame. Emit the Responses terminal failure first so
			...
			BeforeWriteClient: func(msgType coderws.MessageType, payload []byte, turnWroteDownstream bool) error {
				...
				if eventType == "response.created" {
					failureAccountSideEffectsApplied = false
					if responseID := openAIWSPassthroughResponseID(payload); responseID != "" {
						// Capture the upstream id before the frame is written. If this
						// turn dies before visible output, the synthesized response.failed
						// must still identify the response already shown to the client.
						activeResponseID.Store(responseID)
					}
				}
				...
				canReplayInitialTurn := completedTurns.Load() == 0
				if !turnWroteDownstream {
					if isPreOutputRateLimit {
					...
					shouldFailover := eventType == "response.failed" && openAIStreamFailedEventShouldFailover(payload, failedMessage)
					if eventType == "error" {
						shouldFailover = openAIStreamErrorEventShouldFailover(payload, failedMessage)
					}
					if shouldFailover && canReplayInitialTurn {
						return s.newOpenAIStreamFailoverError(
							c, account, true, openAIWSHeaderValueForLog(handshakeHeaders, "x-request-id"),
							payload, failedMessage, handshakeHeaders,
						)
					}
				}
				if (eventType == "error" || eventType == "response.failed") && !failureAccountSideEffectsApplied {
	...
	if errors.Is(relayErr, openaiwsv2.ErrDownstreamStageLimit) && !relayExit.WroteDownstream {
		failoverErr := s.newOpenAIStreamFailoverError(
			c, account, true, openAIWSHeaderValueForLog(handshakeHeaders, "x-request-id"), nil,
			"OpenAI WebSocket first-output staging limit exceeded", handshakeHeaders,
		)
...
func openAIWSPassthroughRelayClientClose(exit openaiwsv2.RelayExit) (coderws.StatusCode, string, bool) {
...
// openAIWSPassthroughShouldEmitFailureEvent limits synthesized terminal
// events to unexpected upstream read failures. Client cancellation, policy
// closes, and graceful shutdown already have an intentional close path and
// must not receive a duplicate response.failed event. Upstream read timeouts
// are included because they terminate an in-flight response without a
